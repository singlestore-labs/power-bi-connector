...
- Order inside the job: verify the ODBC MSI → sign the connector MSI → build the bundle from the signed MSIs (`build-bundle.ps1`) → detach the Burn engine, sign it, reattach it, sign the bundle (`sign-bundle.ps1`) → verify everything (`verify-signature.ps1`, which runs both `Get-AuthenticodeSignature` and `signtool verify /pa /all /v`).
